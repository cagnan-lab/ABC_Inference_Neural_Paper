    ...
    if ~isempty(list)
        for ii = 1:size(list,2)
            for i = 1:N
                for j = 1:M
                    for k = 1:O
                        CSD_bank(:,i,j,k,ii) = permMod.feat_rep{list(ii)}(cond,i,j,k,:);
                    end
                end
            end
        end
        ...
        for i = 1:N
            for j = 1:M
                ...
                if R.plot.cmplx
                    ...
                    hold on
                    for L = lr
                        dl = plot(R.data.feat_xscale,real(squeeze(R.data.feat_emp(cond,i,j,L,:))),'color',[0 0 0],'LineWidth',1.5); hold on
                        dl = plot(R.data.feat_xscale,imag(squeeze(R.data.feat_emp(cond,i,j,L,:))),'color',[0 0 0],'LineWidth',1.5,'LineStyle','--'); hold on
                    end
                    ...
                    hold on
                    for L = lr
                        dl = plot(R.data.feat_xscale,abs(squeeze(R.data.feat_emp(cond,i,j,L,:))),'color',[0 0 0],'LineWidth',1.5); hold on
                    end
                end
                if i == j
                    ylim([0 5])
